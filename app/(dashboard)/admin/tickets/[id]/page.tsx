  ...
  return (
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        <a href="/admin/tickets" className="text-sm text-[#84818A] hover:text-[#2E2C34]">← Back to tickets</a>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-gray-50">
          <div className="w-6 h-6 rounded-full bg-[#7F56D8]/20 flex items-center justify-center text-xs font-semibold text-[#7F56D8]">
            {ticket.user.name.charAt(0)}
          </div>
          <span className="text-xs text-[#84818A]">{ticket.user.name} · {ticket.user.email}</span>
        </div>
      </div>

      {ticket.replies.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-[#2E2C34]">Conversation</h3>
          {ticket.replies.map((reply) => (
            <div
              key={reply.id}
              className={`rounded-xl border p-4 ${
                reply.author.role === "ADMIN"
                  ? "bg-[#7F56D8]/5 border-[#7F56D8]/20 ml-8"
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-[#84818A] uppercase tracking-wide">{reply.author.role}</span>
      ...
      <AdminReplyForm
        ticketId={ticket.id}
        ticketContext={`${ticket.title}: ${ticket.body}`}
